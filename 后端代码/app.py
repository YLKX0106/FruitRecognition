...
def predict_img(path):
    class_names = ['苹果', '香蕉', '奇异果', '芒果', '橘子', '桃子', '梨']
...
def predict():
    ...
    if file and allowed_file(file.filename):
        ...
        result, resultin = predict_img(os.path.join(FILE_DIR, filename))
        resultin="%.2f%%" % ((resultin) * 100)

        userid = request.form.get("userid")
        db = MysqlUtil()
        sql = f'INSERT INTO image VALUES (null,"{str(os.path.join(FILE_DIR, filename))}","{result}","{userid}","{resultin}")'
        ...
        if logr == 1 and imager == 1:
            ...
            msg = "添加失败"
        return jsonify({'result': result, 'msg': msg, 'resultin': resultin})
    else:
...
def loglist(page, pagesize):
    ...
    sql = f'SELECT log.id,imageid,imagepath,imageRec,addTime,resultin from log,image where log.userid={userid} and log.imageid=image.id ORDER BY addTime DESC LIMIT {(page - 1) * pagesize},{pagesize}'
    result = db.fetchall(sql)
    for i in result:
...
if __name__ == '__main__':
    if not os.path.exists(FILE_DIR):
        ...
        os.makedirs(os.path.join(FILE_DIR, 'result'))
